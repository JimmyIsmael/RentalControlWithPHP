<?php
	    ...
	    if($connect)
	    {
		    	 $sql="insert into t_pagos (f_id_usuario,f_id_propiedad,f_gasto,f_detalle,f_fecha) 
						values
	         ...
	         fclose($fp);		
		    	 ...
		    	 if($res)
		    	 {
		    	 	 $sql="insert into t_detalle_pago (f_id_pago,f_monto,f_id_gasto) 
						values
						((select f_id from t_pagos order by f_id desc limit 1),'$monto','$servicio')";
				    	 $res=pg_query($connect, $sql) or die("Error en la consulta SQL");
				    	 $reg=pg_num_rows($res);
				    	 $row = pg_fetch_array ($res);
				    	 if($res)
