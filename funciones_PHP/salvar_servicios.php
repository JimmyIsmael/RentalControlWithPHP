<?php
	...
	$desc_servicio=$_POST['campo_desc'];
	    ...
	    if($connect)
	    {
	    	    if ($id_servicio!="")
	    	    {
		    	     $sql="select * from public.t_servicios where f_id='$id_servicio'";
			    	 $res=pg_query($connect, $sql) or die("Error en la consulta SQL");
			    	 $reg=pg_num_rows($res);
			    	 $row = pg_fetch_array ($res);
			    	 if($reg==1)
			    	 {
			    	   $sql="update public.t_servicios  SET f_descripcion = '$desc_servicio' WHERE f_id = '$id_servicio'";
		    	 ...
		    	 {
		    	   $sql="insert into public.t_servicios (f_descripcion) values ('$desc_servicio')";
		    	   $res=pg_query($connect, $sql) or die("Error insertando datos"); 
		    	   if($res)
		    	   {
